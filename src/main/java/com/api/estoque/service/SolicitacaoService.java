    ...
    public class SolicitacaoService {
        ...
        public SolicitacaoResponse criarSolicitacao(SolicitacaoRequest request, Usuario usuarioLogado) {
            // 1. Valida se a DATA DE ENTREGA (que ainda é obrigatória) foi preenchida
            if (request.dataPrevisaoEntrega() == null) {
                throw new BusinessException("A data de previsão de entrega é obrigatória para submeter uma solicitação.");
            }

            // 2. Valida a ordem das datas, MAS SÓ SE a data de devolução existir (não for null)
            if (request.dataPrevisaoDevolucao() != null &&
                    request.dataPrevisaoDevolucao().isBefore(request.dataPrevisaoEntrega())) {

                throw new BusinessException("A data de devolução (quando fornecida) não pode ser anterior à data de entrega.");
            }
        ...
        public SolicitacaoResponse enviarRascunho(Long id) {
            ...
            // 1. Valida se a DATA DE ENTREGA (que ainda é obrigatória) foi preenchida
            if (rascunho.getDataPrevisaoEntrega() == null) {
                throw new BusinessException("Não é possível enviar a solicitação. A data de previsão de entrega é obrigatória.");
            }

            // 2. Valida a ordem das datas, MAS SÓ SE a data de devolução existir (não for null)
            if (rascunho.getDataPrevisaoDevolucao() != null &&
                    rascunho.getDataPrevisaoDevolucao().isBefore(rascunho.getDataPrevisaoEntrega())) {
